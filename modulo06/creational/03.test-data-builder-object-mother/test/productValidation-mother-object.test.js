...
describe('Test Mother Object', () => {
    ...
    describe('Product Validation Rules', () => {
        it('should return an object error when crating a product with invalid id', () => {
            const product = ProductMotherObject.withInvalidId();
            const result = productValidator(product);
    
            const expected = {
                errors: ["id: invalid length, current [1] expected to be between 2 an 20"],
                result: false
            };
    
            expect(result).to.be.deep.equal(expected);     
        });
        it('should return an object error when crating a product with invalid name', () => {
        ...
        it('should return an object error when crating a product with invalid price', () => {
        ...
        it('should return an object error when crating a product with invalid category', () => {
            const product = ProductMotherObject.withInvalidCategory();
            const result = productValidator(product);
    
            const expected = {
                errors: ["category: invalid category, current[mechanic] expected to be either electronic or organic"],
                result: false
            };
