...
describe(' #Integration - Files - Files Structure', () => {    
    ...
    test('Repository class should have create, read, update and delete methods', async () => {
        ...
        const expectNotImplemented = fn => expect(() => fn.call()).rejects.toEqual("method not implemented yet");
    ...
    test('Service should have the same signature or repository and call its methods', async () => {
        const myConfig = {
            ...config,
       ...
       .forEach(method => jest.spyOn(repository, method).mockRejectedValue());
